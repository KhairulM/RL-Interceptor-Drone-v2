...
class InterceptController:
    ...
    # -- main flight loop ----------------------------------------------------
    def _fly(self, cf: Crazyflie) -> None:
        ...
        if self.takeoff:
            print(f"[intercept] Open-loop takeoff: thrust={self.takeoff_thrust} "
                  f"for {self.takeoff_duration:.1f}s")
            steps = max(1, int(self.takeoff_duration / self.control_dt))
            for _ in range(steps):
                cf.commander.send_setpoint(0.0, 0.0, 0.0, self.takeoff_thrust)
            ...
            with SyncCrazyflie(self.uri, cf=cf) as scf:
                ...
                if self.evader_source == "cf":
                    ...
                    evader_scf = SyncCrazyflie(
                        self.evader_uri, cf=Crazyflie(rw_cache=self.rw_cache))
...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--evader-source", default="scripted",
                   choices=["scripted", "cf"],
                   help="Evader from an internal trajectory or a second Crazyflie.")
    p.add_argument("--evader-uri", default="",
                   help="Crazyflie URI of the evader (when --evader-source=cf).")
    p.add_argument("--max-thrust-pwm", type=float, default=60000.0,
    ...
    p.add_argument("--takeoff-thrust", type=int, default=50000,
                   help="Thrust PWM used during the open-loop takeoff.")
    p.add_argument("--takeoff-duration", type=float, default=1.0,
